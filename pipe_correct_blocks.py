    ...
    def final_matrix(self):
        ...
        for x in range(self.length):
            for y in range(self.length):
                ...
                elif y == self.length - 1 and x != self.length - 1:
                    final_string += dict.get(tuple(self.matrix[x][y])) + "\n"
                else:
                    final_string += dict.get(tuple(self.matrix[x][y]))
        return final_string
...
class PipeMania(Problem):
    ...
    def actions(self, state: PipeManiaState):
        ...
        while index < len(state.board.only_way_actions):
            ...
            index += 1
        action_list = []
        for row in range(state.board.length):
            col = 0
    ...
    def goal_test(self, state):
        return state.board.correct_blocks == (state.board.length * state.board.length)
    
    def h(self, node: Node):
        # Função heuristica utilizada para a procura A*.
        return (node.state.board.length * node.state.board.length) - node.state.board.correct_blocks
